...
export function createMark<T extends object>(
  ...
  initialize: (instance: T, context: Context) => PropertyDescriptor
  ...
  const marker = (proto: object, name: PropertyKey, descriptor?: PropertyDescriptor): void => {
    if (get(proto).has(name)) return
      validate({name, kind: getType(descriptor), access: descriptor || {}})
    get(proto).add(name)
  }
  ...
  return [
    ...
    (instance: T): void => {
      for (const name of getMarks(instance)) {
        ...
        const newDescriptor = initialize(instance, {name, kind: getType(access), access})
